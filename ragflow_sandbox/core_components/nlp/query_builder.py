...
class QueryBuilder:
    ...
    def build_es_query(self, txt, min_match=0.6):
        ...
        """
        txt = self.sub_special_char(rag_tokenizer.tradi2simp(rag_tokenizer.strQ2B(txt.lower())))
        keywords = []
        ...
        for tt in segments:
            ...
            for tk, w in sorted(twts, key=lambda x: x[1] * -1):
                ...
                if tk_syns:
                    tk_syns_str = " ".join([f'"{s}"' if " " in s else s for s in tk_syns])
                    tk_clean = f"({tk_clean} OR ({tk_syns_str})^0.2)"
                    
                if sm:
                    sm_str = " ".join(sm)
                    tk_clean = f'{tk_clean} OR "{sm_str}" OR ("{sm_str}"~2)^0.5'
                    
                if tk_clean.strip():
                    tms.append(f"({tk_clean})^{w:.4f}")
