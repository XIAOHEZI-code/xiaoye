...
def build_system_prompt(deep_mode: bool, memory_context: str | None, vlm_context: str | None) -> SystemMessage:
    ...
    if deep_mode:
        ...
        parts.append(
            ...
            "【高级工具加载原则】：如果你发现问题涉及复杂的冶金机理、工艺因果关系，或是基础检索找不到答案，你必须调用 search_available_tools 搜索高级图谱增强或沙盒工具后，再进行解答！\n"
            "【快速模式】：优先使用基础检索回答。\n"
            "**强制要求**：\n"
            "1. 查到资料后，务必直接给出具体数据，而不是文字堆砌。\n"
            "2. 如果工具返回了图片Markdown格式信息（如 `![图表](http...)`），必须原封不动地将其插入到回答中！严禁编造文献库中没有的数据。"
        )
