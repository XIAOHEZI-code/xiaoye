...
- [x] **重写 `run_worker_pipeline` 签名**：允许传入 `task_id`，以便绑定 SSE 推送管道。
- [x] **接入 `astream_events` (LangGraph)**：监听 `on_chat_model_stream` 和 `on_tool_start`/`on_tool_end` 事件。
- [x] **SSE 事件编解码器映射**：
  - 工具调用 -> `patch: "\n> 🛠️ [系统思考中] 正在调用工具..."`
  - LLM Token 生成 -> `patch: chunk.content`
- [x] **Evaluator 反馈外显**：如果 Evaluator 驳回了当前的检索结果，通过 SSE 提示前端用户 `patch: "\n> 🤔 数据不充分，正在尝试更换策略重新检索..."`。

## 🎯 Phase 2: Delivery 核心管线的切换
目标：删除临时硬编码逻辑，实现真正的智能体派发。

- [x] **清空旧逻辑**：移除 `chat_worker.py` 中写死的 `semantic_search` 工具调用和 `ChatOpenAI` 原生调用流。
- [x] **Prompt 整合转移**：把刚才在 `chat_worker.py` 中写好的优秀提示词（如“直接给出具体数据、渲染图注 Markdown”）平滑转移到 `reasoning/graph.py` 中的 `ResearcherNode`。
- [x] **参数透传**：确保原有的 `memory_context`（硬盘长程记忆）和对话历史被组装成连贯的 `SystemMessage` 传入 Graph 的初始 State 中。
- [x] **异常与超时兜底**：LangGraph 循环极其消耗资源，加入防止死循环的最终阀门保障，即使 Evaluator 连续判定失败，在达到最大重试次数时也必须给出总结回答。
- [x] **编写单元测试**：编写 `tests/test_reasoning_sse.py` 并保证所有用例通过。
